...
Not every dataset reports `UNIT`. The coal price index above has no `UNIT` column, for example; its unit is implied by the `DATA_TRANSFORMATION` dimension. Look up such codes with `imf_get_codelists`.
