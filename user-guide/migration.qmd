...
**Attribute columns.** `imf_dataset` discarded the SDMX attributes the API sends with each observation. `imf_get` returns them as extra columns after `OBS_VALUE`, including `UNIT` and `SCALE`. Pass `attributes=False` to get the old column set.
